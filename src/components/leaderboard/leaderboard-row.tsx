...
export function LeaderboardRow({ player, reduceMotion }: LeaderboardRowProps) {
  return (
    <Link
      aria-label={`Ver Mi Mundial de ${player.name}. Puesto ${player.rank}, ${player.totalPoints.toLocaleString("es-AR")} puntos`}
      className="block outline-none focus-visible:ring-[4px] focus-visible:ring-prode-black focus-visible:ring-offset-[3px] focus-visible:ring-offset-prode-paper"
      href={`/jugadores/${player.id}/mi-mundial`}
      title={`Ver Mi Mundial de ${player.name}`}
    >
      <motion.div
